...
final readonly class RequestStudentAchievementRevision
{
    ...
    public function execute(?Authenticatable $actor, string $id, string $verificationNote, ?string $correlationId = null): ?StudentAchievementData
    {
        ...
        if ($achievement->status !== 'submitted') {
            throw new StudentAchievementMutationException(
                'Revisi prestasi hanya bisa diminta setelah prestasi disubmit.',
        ...
        if ($actorId === null) {
            throw new StudentAchievementMutationException(
                'Actor wajib tersedia untuk meminta revisi prestasi.',
        ...
        return $this->activities->publish(
            actorId: $actorId,
            action: 'prestasi_santri.achievement.revision_requested',
            subjectType: 'student_achievement',
            mutation: fn (): ?StudentAchievementData => $this->repository->requestRevision($id, $verificationNote, $actorId),
