...
- [ ] T09.2 Implementasikan API create/update/delete role.
...
- [ ] T09.3 Implementasikan API assignment dan revoke role user.
...
- [ ] T09.4 Implementasikan API direct permission user.
...
- [ ] T09.5 Implementasikan API impersonation.
...
- [ ] T09.6 Tutup route matrix dan forbidden-controller logic.
...
- [ ] Route matrix cocok dengan API specification.
- [ ] Success/error envelope canonical dan secret-safe.
- [ ] Positive/negative authorization, validation, scope, rate, idempotency,
  serta audit test lulus.
- [ ] Full backend suite lulus.

## Phase 4 - CI dan Open Risk

- [ ] T10.1 Perkuat install deterministik, static analysis, dan coverage.
...
- [x] T10.2 Jalankan rehearsal migration pada MySQL lokal terisolasi.
  ...
  - Batasan: rehearsal ini belum membuktikan upgrade dari snapshot release lama
    karena fixture upgrade belum tersedia; risiko tersebut tetap pada T10.3.

- [ ] T10.3 Tambah migration fresh dan upgrade lane pada CI MySQL terisolasi.
...
- [ ] T11.1 Tambah Vitest untuk frontend critical logic/component.
...
- [ ] T11.2 Tambah fondasi Playwright dan axe-core.
  ...
  - Acceptance: smoke publik/login dan satu axe scan lulus pada dua viewport;
    trace/video hanya tersimpan saat gagal dan masuk allowlist aman.
  - Command/test: focused Playwright smoke desktop/mobile.
  - Dependency: T11.1 dan izin dependency.

- [ ] T11.3 Tambah critical flow empat module pada Playwright/axe.
...
- [ ] T11.4 Integrasikan browser/accessibility gate ke CI.
...
- [ ] T11.5 Tambah CodeQL, dependency scan, dan artifacts.
...
- [ ] T11.8 Verifikasi flow interaktif browser manual.
...
- [x] T12.1 Putuskan dan perbaiki exact-version constraint Composer.
  ...
  - Evidence: empat constraint memakai caret range; `composer update --lock
    --no-install --no-audit --no-scripts` tidak mengubah package; lock hanya
    berubah pada `content-hash`; `composer validate --strict` lulus.

### Checkpoint 4 - Release Gate

- [ ] Backend, frontend, MySQL migration, browser/a11y, security, dan artifact
  jobs lulus.
- [ ] Browser manual serta Playwright mempunyai evidence.
- [ ] Composer/npm audit bersih pada severity yang diwajibkan.
- [ ] Build production lulus.

## Phase 5 - Penutupan dan Evidence

- [ ] T12.2 Jalankan final verification dan sinkronkan downstream docs.
...
- [ ] Scope CI dan open risk selesai.
