...
type Props = {
    meta: StudentDisciplineIndexPageProps['cases']['meta'];
    pagination: StudentDisciplineIndexPageProps['pagination'];
    onPageChange: (page: number) => void;
    onPerPageChange: (perPage: number) => void;
};

export function KedisiplinanSantriPagination({
    meta,
    pagination,
    onPageChange,
    onPerPageChange,
    ...
    return (
        <div className="flex flex-col gap-3 border-t pt-4 text-sm sm:flex-row sm:items-center sm:justify-between">
            <p className="text-foreground/65">
                Menampilkan {firstItem}-{lastItem} dari {meta.total} kasus -
            ...
            <div className="flex flex-wrap items-center gap-2">
                ...
                >
                    <SelectTrigger
                        aria-label="Jumlah kasus per halaman"
                        className="w-28"
                    >
                        <SelectValue />
                    </SelectTrigger>
