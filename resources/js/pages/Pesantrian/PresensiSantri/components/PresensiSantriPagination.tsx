...
type Props = {
    meta: StudentAttendanceIndexPageProps['attendances']['meta'];
    pagination: StudentAttendanceIndexPageProps['pagination'];
    onPageChange: (page: number) => void;
    onPerPageChange: (perPage: number) => void;
};

export function PresensiSantriPagination({
    meta,
    pagination,
    onPageChange,
    onPerPageChange,
    ...
    return (
        <div className="flex flex-col gap-3 border-t pt-4 text-sm sm:flex-row sm:items-center sm:justify-between">
            <p className="text-foreground/65">
                Menampilkan {firstItem}-{lastItem} dari {meta.total} sesi
                presensi · halaman {meta.currentPage} dari {meta.lastPage}
            </p>
            <div className="flex flex-wrap items-center gap-2">
                ...
                >
                    <SelectTrigger
                        aria-label="Jumlah sesi presensi per halaman"
                        className="w-28"
                    >
                        <SelectValue />
                    </SelectTrigger>
