...
type Props = {
    meta: StudentIndexPageProps['students']['meta'];
    pagination: StudentIndexPageProps['pagination'];
    onPageChange: (page: number) => void;
    onPerPageChange: (perPage: number) => void;
};

export function SantriPagination({
    meta,
    pagination,
    onPageChange,
    onPerPageChange,
    ...
    return (
        <div className="flex flex-col gap-3 border-t pt-4 text-sm sm:flex-row sm:items-center sm:justify-between">
            <p className="text-foreground/65">
                Menampilkan {firstItem}-{lastItem} dari {meta.total} santri ·
